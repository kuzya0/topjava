...
public class MealsUtil {
    ...
    public static List<MealWithExceed> getTimeFilteredWithExceeded(Collection<Meal> meals, int caloriesPerDay, LocalTime startTime, LocalTime endTime) {
        return getFilteredWithExceeded(meals, caloriesPerDay, meal -> DateTimeUtil.isBetween(meal.getTime(), startTime, endTime));
    }

    public static List<MealWithExceed> getDateFilteredWithExceeded(Collection<Meal> meals, int caloriesPerDay, LocalDate startDate, LocalDate endDate) {
        return getFilteredWithExceeded(meals, caloriesPerDay, meal -> meal.getDate().isAfter(startDate) && meal.getDate().isBefore(endDate));
    }
